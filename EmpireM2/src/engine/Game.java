...
public class Game {
	...
	public Game(String playerName, String playerCity) throws IOException {
		logAutoResolve = new JFrame();
		news = new JTextArea();
		news.setPreferredSize(new Dimension(600, 600));
		...
		logAutoResolve.setLayout(new FlowLayout());

		logAutoResolve.add(logPanel);
		logAutoResolve.setVisible(false);
		logAutoResolve.setSize(1080, 600);
	...
	public void setVisibilty() {
		logAutoResolve.setVisible(true);
	}
	...
	public void autoResolve(Army attacker, Army defender) throws FriendlyFireException {
		...
		while (attacker.getUnits().size() != 0 && defender.getUnits().size() != 0) {
			...
			} else if (unit1 instanceof Infantry)
				type = "Infantry";
			else
				type = "Cavalry";
			...
			} else if (unit2 instanceof Infantry)
				type2 = "Infantry";
			else
				type2 = "Cavalry";

			news.setText(news.getText() + "Attacking Unit: " + type + " " + unit1.getCurrentSoldierCount() + "  "
					+ unit1.getLevel());
			news.setText(news.getText() + "\n");
			news.setText(news.getText() + "Defending Unit: " + type2 + " " + unit2.getCurrentSoldierCount() + "  "
					+ unit2.getLevel());
			news.setText(news.getText() + "\n" + "Before Attack\n");
			logPanel.add(news);
			if (turn == 1) {

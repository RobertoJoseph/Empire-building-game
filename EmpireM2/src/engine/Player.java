...
public class Player {
	...
	public void downgradeBuilding(ArcheryRange b)
			throws BuildingInCoolDownException, MinLevelException, DowngradeLimitExeption {
		...
		if (b.isCoolDown()) {
			...
			if (b.getLevel() == 1) {
				...
				if (getDowngradeLimit() >= 4) {
					throw new DowngradeLimitExeption();
				} else {
					b.downgrade();
					setDowngradeLimit(getDowngradeLimit() + 1);
	...
	public void laySiege(Army army, City city) throws TargetNotReachedException, FriendlyCityException {
		...
		else if (!army.getCurrentLocation().equals(city.getName()))
